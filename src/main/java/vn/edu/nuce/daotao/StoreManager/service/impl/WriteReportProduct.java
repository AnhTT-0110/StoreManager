...
@Log4j2
public class WriteReportProduct {

    public static final int COLUMN_INDEX_ID = 0;
    public static final int COLUMN_INDEX_NAME = 1;
    public static final int COLUMN_INDEX_QUANTITY = 2;
    public static final int COLUMN_INDEX_DCASH = 3;
    private static CellStyle cellStyleFormatNumber = null;

    public static CodeSystem writeExcel(List<ProcedureReportProduct> procedureReportBills, String excelFilePath) {
        try {
            ...
            rowIndex++;
            for (ProcedureReportProduct book : procedureReportBills) {
    ...
    private static void writeBook(ProcedureReportProduct reportBill, Row row) {
        ...
        Cell cell = row.createCell(COLUMN_INDEX_ID);
        cell.setCellValue(reportBill.getProductCode());

        cell = row.createCell(COLUMN_INDEX_NAME);
        cell.setCellValue(reportBill.getProductName());

        cell = row.createCell(COLUMN_INDEX_QUANTITY);
        cell.setCellValue(Double.valueOf(reportBill.getQuantity().toString()));
        cell.setCellStyle(cellStyleFormatNumber);

        cell = row.createCell(COLUMN_INDEX_DCASH);
        cell.setCellValue(Double.valueOf(reportBill.getTotal().toString()));
        cell.setCellStyle(cellStyleFormatNumber);
